...
- `npm run lint`·`tsc --noEmit` 통과 확인. Playwright로 1000건 데이터에서 특정 두 카드 사이로 정확히 드롭되는지, 새로고침 후에도 그 위치가 유지되는지 직접 검증. 별도로 같은 두 이웃 사이에 70회 연속 삽입하는 스트레스 스크립트로 정밀도 소진 → 자동 재정렬 → 이후에도 전체 순서 무결성 유지(중복/역전 없음)를 직접 확인 (실제로 30~40회 근처에서 재정렬이 발동하는 것을 로그로 확인)
