# CONVENTION.md

이 프로젝트의 커밋/작업 진행 방식을 정의합니다.

## 진행 방식

- 요구사항을 기능 단위로 쪼개서 점진적으로 구현하고, 기능 단위마다 커밋한다.
- 커밋 히스토리 자체가 작업 기록이 되도록 유지한다 (스쿼시/리라이트 지양).
- 커밋 1개는 [PROMPTS.md](./PROMPTS.md)의 섹션 1개와 1:1로 대응된다.

## 커밋 메시지 형식

```
type(scope): 요약

- 무엇을 했는지 / 왜 그렇게 했는지
- AI 초안을 어떻게 검토·보완했는지 (그대로 채택했다면 그 사실도 명시)
```

### type

| type | 의미 |
| --- | --- |
| feat | 새로운 기능 |
| fix | 버그 수정 |
| refactor | 동작 변경 없는 코드 개선 |
| test | 테스트 추가/수정 |
| docs | 문서 변경 |
| chore | 빌드/설정/의존성 등 잡무 |

### scope

기능 단위 식별자. [PROMPTS.md](./PROMPTS.md)의 섹션 제목 `## [scope] 기능 설명` 과 동일한 값을 사용해서,
커밋과 프롬프트 기록이 1:1로 대응되게 한다.

예: `stage-move`, `candidate-card`, `mock-api`

### 본문 규칙

- "무엇을 왜" 했는지 남긴다.
- AI가 만든 초안을 그대로 썼는지, 어떤 부분을 검토해서 고쳤는지 최소 한 줄은 남긴다.

### 예시

```
feat(stage-move): 카드 단계 이동 + mock API 저장

- 드래그 대신 액션 버튼 방식 선택
- AI 초안은 로컬 상태만 갱신 -> API persist 누락, 직접 보완
```

## 커밋 단위 기준

- 하나의 커밋은 하나의 scope(기능)에 대응한다.
- 문서 정비(CONVENTION/PROMPTS/DECISIONS/README)는 `docs` 또는 `chore`로 별도 커밋한다.
- 여러 기능을 한 커밋에 섞지 않는다.

## PROMPTS.md 작성 규칙

- PROMPTS.md는 기능 단위로 AI에게 준 프롬프트와 그 결과에 대한 리뷰/검증만 기록하는 문서다. 안내문·템플릿은 두지 않는다.
- 섹션은 커밋 히스토리와 1:1로 대응되며, 커밋 메시지의 scope와 섹션 제목의 scope를 동일하게 맞춘다.
- 섹션 제목: `## [scope] 기능 설명`
- "프롬프트"는 요약이 아니라 실제로 입력한 문구를 그대로 기록한다.
- "AI 출력 요지"는 매 요청마다 AI가 채워서 갱신한다.
- "리뷰 / 검증"은 사람이 직접 판단하는 항목이므로, AI는 알 수 있는 범위만 간단히 정리하고
  나머지는 공란으로 남겨서 직접 채운다.
- 아래 템플릿은 각 항목에 어떤 내용이 들어가야 하는지 보여주는 지침이다. 실제로 항목을 쓸 때는
  템플릿 문구를 그대로 옮기지 말고, 그 자리에 실제 내용을 채운다.

```
## [scope] 기능명

### 프롬프트 1
> (AI에 준 지시 - 요약이 아닌 실제 문구)

### AI 출력 요지
- (AI가 무엇을 내놓았는지)

### 리뷰 / 검증
- (무엇이 문제였나: 틀림 / 비효율 / 요구 누락)
- (어떻게 알아챘나: 직접 재현 / 테스트 / 정독)
- (그대로 채택 / 수정 / 기각 -> 어떤 판단을 왜 했나)
- (수정했다면 어떻게 고쳤나)
```

## DECISIONS.md 작성 규칙

- DECISIONS.md는 설계 결정만 기록하는 문서다. 안내문·템플릿은 두지 않는다.
- 특정 디자인 패턴 채택, 명시적인 구조 설계·변경점 등 굵직한 결정만 기록한다. 기능 하나 구현할 때마다 나오는 자잘한 구현 디테일은 적지 않는다 (그런 내용은 PROMPTS.md의 "AI 출력 요지"에 남긴다).
- 결정이 내려질 때마다 AI가 "결정 기록" 절에 초안을 추가하고, 세부 내용은 이후 직접 다듬는다.
- 아래 템플릿은 지침이며, 실제 항목에는 템플릿 문구 대신 실제 결정 내용을 채운다.

```
### (결정 제목)
- 결정: (실제로 무엇을 정했는지)
- 이유: (그렇게 정한 이유)
- 대안 및 기각 사유: (검토했지만 채택하지 않은 대안과 그 이유)
```

## 연관 문서

- [PROMPTS.md](./PROMPTS.md): 기능별 프롬프트와 리뷰/검증 기록 (커밋과 1:1 대응)
- [DECISIONS.md](./DECISIONS.md): 설계 결정 기록
- [README.md](./README.md): 실행 방법 및 스택 안내
